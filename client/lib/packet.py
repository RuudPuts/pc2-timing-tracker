...
class Packet(object):
    ...
    def __init__(self, build_version, sequence_number, packet_type, buffer):
        self.build_version = build_version
        self.sequence_number = sequence_number
        self.packet_type = packet_type
        if hasattr(self.__class__, "STRUCTURE"):
            self._data = self.__class__.STRUCTURE.read_dict(buffer)
        else:
            self._data = {}

    def bytes_to_string(self, bytes):
        value = str(bytes, encoding='iso_8859_1', errors='surrogateescape')

        return value[:value.index('\x00')]

    @staticmethod
    def read_from(buffer: BytesIO):
        header = Packet.HEADER.read_dict(buffer)
        build_version = header["buildVersion"]
        sequence_number = (header["seq_packet"] & 0xFC) >> 2
        packet_type = header["seq_packet"] & 0x3
        packet_class = PACKET_CLASSES.get(packet_type, Packet)

        # print(" > build_version: {}".format(build_version))
        # print(" > sequence_number: {}".format(sequence_number))
        # print(" > Parsing {}".format(packet_class.__name__))

        return packet_class(build_version, sequence_number, packet_type, buffer)
...
class TelemetryPacket(Packet):
    ...
    def __init__(self, build_version, sequence_number, packet_type, buffer):
        super(TelemetryPacket, self).__init__(build_version, sequence_number, packet_type, buffer)
        self._data["tyres"] = [{}, {}, {}, {}]

        for datapoint in TelemetryPacket.TYRES_STRUCTURE:
            self.read_tyre_data(datapoint, buffer)

        self._data.update(TelemetryPacket.EXTRAS_WEATHER_STRUCTURE.read_dict(buffer))

        self._data["participants"] = []

        for _ in range(0, 56):
            p = TelemetryPacket.PARTICIPANT_INFO_STRUCTURE.read_dict(buffer)
        ...
        self._data.update(TelemetryPacket.EPILOGUE_STRUCTURE.read_dict(buffer))
    ...
    def read_tyre_data(self, datapoint, buf):
        data_field = binio.new([datapoint])
        for i in Tyres:
            self._data["tyres"][i.value][datapoint[2]] = data_field.read_dict(buf)[datapoint[2]]
...
class ParticipantInfoStringsPacket(Packet):
    ...
    NAME_STRUCTURE = binio.new([(
        64, binio.types.t_byte, "name")
    ])

    FASTEST_LAP_TIME_STRUCTURE = binio.new([(1, binio.types.t_float32, "fastestLapTime")])

    def __init__(self, build_version, sequence_number, packet_type, buf):
        super(ParticipantInfoStringsPacket, self).__init__(build_version, sequence_number, packet_type, buf)

        self._data["carName"] = self.bytes_to_string(self._data["carName"])
        self._data["carClassName"] = self.bytes_to_string(self._data["carClassName"])
        self._data["trackLocation"] = self.bytes_to_string(self._data["trackLocation"])
        self._data["trackVariation"] = self.bytes_to_string(self._data["trackVariation"])

        self._data["participants"] = []

        for _ in range(0, 16):
            p = ParticipantInfoStringsPacket.NAME_STRUCTURE.read_dict(buf)
            p["name"] = self.bytes_to_string(p["name"])
            self._data["participants"].append(p)
...
class ParticipantInfoStringsAdditionalPacket(Packet):
    ...
    def __init__(self, build_version, sequence_number, packet_type, buffer):
        super(ParticipantInfoStringsAdditionalPacket, self).__init__(build_version, sequence_number, packet_type, buffer)

        self._data["participants"] = []

        for _ in range(0, 16):
            p = ParticipantInfoStringsAdditionalPacket.NAME_STRUCTURE.read_dict(buffer)
            p["name"] = self.bytes_to_string(p["name"])
            self._data["participants"].append(p)
